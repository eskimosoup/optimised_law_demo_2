module Optimadmin
  class FrequentlyAskedQuestionsController < Optimadmin::ApplicationController
    ...
    def index
      @frequently_asked_questions = Optimadmin::BaseCollectionPresenter.new(collection: FrequentlyAskedQuestion.where('question ILIKE ?', "%#{params[:search]}%").page(params[:page]).per(params[:per_page] || 15), view_template: view_context, presenter: Optimadmin::FrequentlyAskedQuestionPresenter)
    end
  ...
  private
    ...
    def frequently_asked_question_params
      params.require(:frequently_asked_question).permit(:position, :question, :answer, :display, service_ids: [])
    end
  end
end
